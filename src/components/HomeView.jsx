...
const HomeView = () => {
      ...
      const fetchEmployees = async () => {
          try {
              const response = await fetch(
                  "http://localhost:5000/search_employees",
                  {
                      method: "POST",
                      credentials: "include",
                  }
              ...
              setShowSnackAlert(
                  "error",
                  "Por favor envia este error a desarrollo: " + error,
                  true
              );
  ...
  const handleOpenModal = (cedula) => {
      ...
      const getJoinInfo = async () => {
          try {
              const response = await fetch(
                  "http://localhost:5000/get_join_info",
                  {
                      method: "POST",
                      credentials: "include",
              ...
              if (data.status === "success") {
                  ...
                  setShowSnackAlert(
                      "error",
                      "Por favor envia este error a desarrollo: " +
                      data.error,
                      true
                  );
              }
  ...
  useEffect(() => {
      ...
      const birthDate = pageInputs
          .flatMap((inputGroup) => inputGroup.inputs)
          .find((input) => input.id === "3")?.value;
      setDataCalculateAge(calculateAge(birthDate));
      const affiliationDate = pageInputs
          .flatMap((inputGroup) => inputGroup.inputs)
  ...
  useEffect(() => {
      const newRows = tableData.map(
          ([cedula, nombre, celular, correo, estado]) => ({
              cedula,
              nombre,
              celular,
              correo,
  ...
  const handleSwitch = (row) => {
      ...
      const changeState = async () => {
          try {
              const response = await fetch(
                  "http://localhost:5000/change_state",
                  {
                      method: "POST",
                      credentials: "include",
                      headers: {
  ...
  const submitEdit = (event) => {
      ...
      const updateTransaction = async () => {
          try {
              const response = await fetch(
                  "http://localhost:5000/update_transaction",
                  {
                      method: "POST",
                      credentials: "include",
                      headers: {
              ...
              if (data.status === "success") {
                  ...
                  setShowSnackAlert(
                      "success",
                      "Edición realizada correctamente"
                  );
              }
  ...
  const submitAdd = (event) => {
      ...
      const insertTransaction = async (formData) => {
          try {
              const response = await fetch(
                  "http://localhost:5000/insert_transaction",
                  {
                      method: "POST",
                      credentials: "include",
                      headers: {
